...
ranker — see ``agents/transformer_v2/pretrain/target_rank.py``), walks
...
from ..history import HISTORY_MAX_LOOKBACK, HISTORY_OFFSETS, N_HISTORY
...
def _stack_history(
    ...
    encoder expects. ``offsets`` is the oldest-first list of "turns ago"
    indices to pick from the deque — for v2 this is
    :data:`HISTORY_OFFSETS`. Slots whose frame is missing (start of
    episode or sparse-window gap before the deque has accumulated
    enough turns) are zero-filled; their masks stay all-False so the
...
def score_replay(
    ...
    *,
    ...
    # Prefer the ckpt's saved offsets so a ckpt trained with a different
    # window than the current code still scores correctly. Fall back to
    # the module constant when the ckpt is older.
    offsets_cfg = cfg.get("history_offsets")
    offsets: tuple[int, ...] = (
        tuple(int(x) for x in offsets_cfg) if offsets_cfg else HISTORY_OFFSETS
    )
    max_planets = int(cfg.get("max_planets", 64))
    max_fleets = int(cfg.get("max_fleets", 1024))

    tracker = FleetTracker()
    # Buffer needs ``max(offsets) + 1`` turns so the oldest slot still
    # finds a frame on a sufficiently long replay.
    history: deque[dict[str, torch.Tensor]] = deque(maxlen=max(offsets) + 1)
    ...
    for t, step in enumerate(replay_steps):
        ...
        stacked = _stack_history(history, offsets, device_t)
