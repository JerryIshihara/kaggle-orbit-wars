"""Per-fleet featurizer for transformer_v2.
    ...
    def to_label_dict(self) -> dict[str, float]:
        ...
        * ``TRANSFORMER_AUX_LABELS`` — depend on planet state and other
        ...
        return {
            ...
            # ---- B. kinematic / spatial preservation (categorical / binary) ----
            ...
            "will_hit_sun": _will_hit_sun(self.x, self.y, self.vx, self.vy),
...
# Sun geometry. Mirrors agents/physics_utils.py (SUN_CX=50, SUN_CY=50,
# SUN_RADIUS=10, SUN_MARGIN=1). Fleets travel in straight lines, so the
# "will hit sun" test is a ray-vs-circle intersection on the no-go disc
# (SUN_RADIUS + SUN_MARGIN = 11).
SUN_CX = 50.0
SUN_CY = 50.0
SUN_NO_GO_RADIUS = 11.0


def _will_hit_sun(x: float, y: float, vx: float, vy: float) -> int:
    """Binary: does the forward ray (x,y) + t·(vx,vy), t>=0, ever enter the
    sun's no-go disc?

    Pure function of the kinematic state — the four inputs are all in the
    fleet feature vector (after denormalization), so the label is
    derivable from the encoder input alone. The current frame is also
    considered "hit" if the fleet is already inside the disc, even though
    the env would have destroyed such a fleet upstream — kept consistent
    so the label is well-defined on edge cases.
    """
    dx = SUN_CX - x
    dy = SUN_CY - y
    r2 = SUN_NO_GO_RADIUS * SUN_NO_GO_RADIUS
    if dx * dx + dy * dy <= r2:
        return 1
    v2 = vx * vx + vy * vy
    if v2 < 1e-9:
        return 0
    t_star = (dx * vx + dy * vy) / v2
    if t_star <= 0.0:
        return 0
    px = x + t_star * vx
    py = y + t_star * vy
    ddx = SUN_CX - px
    ddy = SUN_CY - py
    return 1 if ddx * ddx + ddy * ddy <= r2 else 0
...
# imported it; the source of truth is ``transformer_v2.paths``.
