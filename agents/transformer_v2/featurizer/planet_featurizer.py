"""Per-planet/comet featurizer for transformer_v2.
...
FUTURE_HORIZONS = tuple(range(1, 31))          # 1, 2, …, 30 — 30 dense (every-1-step) anchors
N_FUTURE_ANCHORS = len(FUTURE_HORIZONS)
LOOKAHEAD = 5                                  # for owner_t+5 / net_ships_t+5 labels

# Trajectory extrapolation labels (encoder-pretrain). 1:1 with input
# anchors so the trajectory pretrain task is "preserve the input
# through the encoder bottleneck" — every dense h=1..30 horizon.
EXTRAP_HORIZONS = FUTURE_HORIZONS
...
ANCHOR_DXY_NORM = 50.0                         # board half-width; the original constant — bumping it shrinks precision targets too
...
# Number of channels per trajectory anchor:
#   0: dx_norm
#   1: dy_norm
#   2: valid_bit (0 = path ran out / episode ended; 1 = real anchor)
#   3: horizon_norm = horizon_turns / max_horizon — explicit position
#      encoding. The encoder is a flat MLP (no Conv1d), so it has no
#      built-in sense of "this anchor is at h=10 vs h=30"; the channel
#      makes that distinction available without architectural priors.
TRAJ_CHANNELS = 4
...
class PlanetFeaturizer:
    ...
    def to_vector(self, learner_slot: int, num_players: int) -> list[float]:
        ...
        # Future anchors: (dx_norm, dy_norm, valid, horizon_norm) at each
        # horizon in FUTURE_HORIZONS. ``horizon_norm`` is the 4th channel
        # — h / FUTURE_HORIZONS[-1] — giving the flat encoder MLP an
        # explicit position signal (no Conv1d to provide one).
        max_h = float(FUTURE_HORIZONS[-1])
        for (dx, dy, valid), h in zip(self.future_dxy, FUTURE_HORIZONS):
...
ENCODER_PRETRAIN_LABELS: tuple[str, ...] = (
    ...
    # paired with ``extrap_mask`` for masked MSE. The sparse long-
    # horizon ``extrap_xy_long`` head was dropped: at d=128 the
    # encoder can't service both dense + sparse without diluting
    # per-horizon precision. h=32..50 is also entirely planet-only,
    # which doesn't help the comet-trajectory representation.
    "extrap_trajectory",
)
...
ENCODER_LABEL_HEADS: dict[str, dict[str, Any]] = {
    ...
    # Trajectory decoder: predict (dx, dy) at each of FUTURE_HORIZONS
    # (now 30 every-2-step horizons, h=2..60) relative to current pos.
    # Loss is masked MSE; per-step mask is 0 when the entity is gone at t+h.
    "extrap_trajectory":      {
        "type": "masked_regression",
        "n_horizons": N_EXTRAP_HORIZONS,
        "out_dim": 2 * N_EXTRAP_HORIZONS,
