...
Run from the repo root:
    ...
    python -m agents.transformer_v2.pretrain.fleet_encoder \
...
from ..featurizer.fleet_featurizer import BOARD, MAX_SPEED, _will_hit_sun
from ..paths import FLEET_DATASET_DIR, FLEET_RUNS_DIR
from ..encoder.fleet_encoder import FleetEncoder


# ---------- Backfill missing labels from raw inputs ----------
def _label_from_raw(name: str, raw: list[float]) -> int | float:
    """Compute a pretrain label from the raw 24-dim feature vector.

    Used by :class:`FleetCsvDataset` when an older CSV doesn't carry a
    newer label column. Keeps backward compatibility without forcing a
    full CSV rebuild.

    Layout of ``raw`` (see :meth:`FleetFeaturizer.to_vector`):
      [0] x_norm = x / BOARD       [1] y_norm = y / BOARD
      [2] vx_norm = vx / MAX_SPEED [3] vy_norm = vy / MAX_SPEED
      ... (other channels not needed here)
    """
    if name == "will_hit_sun":
        x = raw[0] * BOARD
        y = raw[1] * BOARD
        vx = raw[2] * MAX_SPEED
        vy = raw[3] * MAX_SPEED
        return _will_hit_sun(x, y, vx, vy)
    raise KeyError(
        f"no backfill rule for missing label {name!r}; rebuild the CSVs "
        f"or add a rule to _label_from_raw."
    )
    ...
    def __init__(self, csv_paths: list[Path]):
        ...
        for path in csv_paths:
            with path.open() as fh:
                ...
                for row in reader:
                    raw = [float(row[c]) for c in self.FEATURE_COLS]
                    feats.append(raw)
                    for name in ENCODER_PRETRAIN_LABELS:
                        ...
                        if name in csv_fields:
                            ...
                            # Fallback for older CSVs that don't carry newer
                            # labels — compute from the raw feature vector.
                            v = _label_from_raw(name, raw)
                        labels[name].append(
...
class FleetEncoderPretrainModel(nn.Module):
    """``FleetEncoder`` + per-label MLP heads.

    Head depth is controlled by ``head_num_layers``:

      * ``1`` — single ``Linear(d_model → out_dim)`` (the original
        "intentionally tiny" design; capped ``distance_to_sun_bucket``
        at ~56% even with strong reconstruction MSE).
      * ``2`` — ``Linear → GELU → Linear`` with hidden width ``d_model``;
        unlocks nonlinear decode (e.g. concentric distance-to-sun
        annuli). Brought ``distance_to_sun_bucket`` from 56% → ~90%.
      * ``3+`` — additional ``GELU + Linear`` stages at hidden width.
        Diminishing returns beyond 2 for these labels in practice, but
        useful to verify the bottleneck location.

    Hidden width defaults to ``d_model`` so head capacity scales with
    the encoder; pass ``head_hidden`` to override.
    """
    ...
    def __init__(
        ...
        self.encoder = FleetEncoder(
            d_model=d_model, num_layers=encoder_num_layers,
        )
        ...
        for name in ENCODER_PRETRAIN_LABELS:
            ...
            self.heads[name] = (
                layers[0] if len(layers) == 1 else nn.Sequential(*layers)
            )
...
def train(
    *,
    ...
    model = FleetEncoderPretrainModel(
        d_model=d_model,
