...
Run from the repo root:
    ...
    python -m agents.transformer_v2.pretrain.pair_score \\
...
class FracHead(nn.Module):
    ...
    ``agents/transformer_v2/runner.py``'s dead-code path for the
...
def prepare_dataset(
    ...
    **via the** ``dataset=`` **kwarg** for every subsequent training
    ...
    """
    # v2 always uses HISTORY_OFFSETS regardless of the caller's value;
    # coerce here so cache keys, ckpt configs, and downstream call sites
    # all agree on N_HISTORY (= 6) without forcing every old notebook to
    # be rewritten.
    if n_history != N_HISTORY:
        import warnings
        warnings.warn(
            f"prepare_dataset: coercing n_history={n_history} -> {N_HISTORY} "
            f"(v2 uses HISTORY_OFFSETS).",
            stacklevel=2,
        )
        n_history = N_HISTORY
