...
prediction (see ``agents/transformer_v2/pretrain/pair_score.py``);
...
and earlier). This file remains so:
  ...
  * ``from agents.transformer_v2 import ppo`` still succeeds (the
...
_DECODER_REMOVED_MSG = (
    "transformer_v2 PPO is currently disabled — the action decoder it "
    "drove was removed during the encoder-freeze + pair-head redesign. "
    "Train the new PairScoreHead first via "
    "`python -m agents.transformer_v2.pretrain.pair_score`; re-enable "
