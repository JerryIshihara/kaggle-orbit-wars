# transformer_v2 — Handoff (PairScoreHead smoke test)
...
L2      CrossEntityAttention (2 layers, 8 heads)  [FROZEN]
                       ▼
...
| Thing | Path |
|---|---|
| Pair-score code | `agents/transformer_v2/pretrain/pair_score.py` |
| Encoder modules (frozen) | `agents/transformer_v2/encoder/{fleet,planet,entity}_encoder.py`, `agents/transformer_v2/aggregator/cross_entity.py` |
| Dataset glue | `agents/transformer_v2/pretrain/expert_action.py` (only `ActionSnapshotDataset` + `FRAC_Z_MIN` survive) |
| Encoder reuse helper | `_entity_tokens_per_step` in `agents/transformer_v2/pretrain/cross_entity.py` |
| Original full design | `agents/transformer_v2/DESIGN.md` (describes the *spec*; code differs — see "Notable gap" near the bottom of that file) |
| Critique of 2026-05-04 | `agents/transformer_v2/CRITIQUE_2026_05_04.md` (untracked workspace note from earlier session — read for prior debugging context) |
...
```bash
.venv/bin/python -m agents.transformer_v2.pretrain.pair_score \
...
1. **`featurizer/action_featurizer.py`** has a 155-line uncommitted modification on disk from before this session. Not mine; left untouched. Look at the diff (`git diff agents/transformer_v2/featurizer/action_featurizer.py`) before doing anything in that file — there's in-progress work there.
...
| File | Why |
|---|---|
| `agents/transformer_v2/HANDOFF.md` | this memo |
| `agents/transformer_v2/CRITIQUE_2026_05_04.md` | most recent prior-session critique of the full stack |
| `tmp/ppo_training_progress.md` | last PPO run's iter-30 snapshot (eval_winrate=0.0) — explains why we're here |
| `agents/transformer_v2/DESIGN.md` | the original spec (note: code diverged into the v2-path; see `pair_score.py` for what's actually live) |
