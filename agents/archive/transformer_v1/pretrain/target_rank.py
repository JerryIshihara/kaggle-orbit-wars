...
Run (CLI):
    ...
    python -m agents.transformer_v1.pretrain.target_rank \\
...
class TargetRanker(nn.Module):
    ...
    Parameter count @ d_model=64, d_rank=128, n_heads=4, mlp_hidden=128,
    mlp_layers=3:
...
def _load_encoder_stack(
    ...
    *,
    ...
    """Load four encoders from an action-stage ckpt. No pair head."""
    ...
    for k in ("fleet_encoder", "planet_encoder", "entity_encoder", "cross"):
        if k not in ckpt:
            raise KeyError(
                f"{encoder_ckpt} missing '{k}' state — expected an action-"
                "stage ckpt with the full encoder set."
            )
    ...
    *,
    ...
    if init_from is not None:
        ...
        if "target_ranker" in prior:
            stack.target_ranker.load_state_dict(prior["target_ranker"])
