...
- **Pre-LN (`norm_first=True`)** — more stable than post-LN for
  shallow stacks. 3 layers without a warmup schedule.
- **3 layers, 4 heads, `ff_mult=2`** — matches entity-token
  complexity. Per-layer QKV is `64 × 64` ≈ 12k params; total under
  100k. Tiny by transformer standards but enough capacity for
  `P ≤ 64` self-attention.
